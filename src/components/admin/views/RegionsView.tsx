                        ...
                        {filtered.map((region) => {
                            ...
                            return (
                                <SwipeableCard key={region.id} actions={[
                                    ...
                                    { icon: 'fa-trash', label: 'Sil', color: 'bg-red-500', onClick: () => { if (confirm('Bu bölgeyi silmek istediğinize emin misiniz?')) { setEditContent({ ...editContent, regions: regions.filter(r => r.id !== region.id) }); showToast('Bölge silindi', 'delete'); } } },
                                ]}>
                                    <div className="p-4">
                                        ...
                                        <div className="flex items-center gap-2 mt-3 pt-2 border-t border-white/[0.04]">
                                            ...
                                            <button onClick={() => { if (confirm('Bu bölgeyi silmek istediğinize emin misiniz?')) { haptic.error(); setEditContent({ ...editContent, regions: regions.filter(r => r.id !== region.id) }); showToast('Bölge silindi', 'delete'); } }}
                                        ...
                                        <tr key={region.id} {...dragProps}
                                            ...
                                            <td className="px-3 py-3.5">
                                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                                                    ...
                                                    <button onClick={() => { if (confirm('Bu bölgeyi silmek istediğinize emin misiniz?')) { setEditContent({ ...editContent, regions: regions.filter(r => r.id !== region.id) }); showToast('Bölge silindi', 'delete'); } }}
