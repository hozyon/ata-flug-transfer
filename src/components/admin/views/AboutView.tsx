    ...
    return (
        <div className="animate-in slide-in-from-right-8 duration-500 space-y-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                    { label: 'Başlık', value: editContent.about.title ? '✓' : '—', icon: 'fa-heading', iconBg: 'bg-[#c5a059]', gradient: 'from-[#c5a059]/15 to-amber-600/5', border: 'border-[#c5a059]/15' },
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl overflow-hidden">
                ...
                <div className="px-6 py-5 border-b border-white/[0.04] flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-[#c5a059]/10 flex items-center justify-center">
                        <i className="fa-solid fa-address-card text-[#c5a059]"></i>
                    </div>
                    <div>
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                                <i className="fa-solid fa-heading text-[8px] text-[#c5a059]"></i> Başlık
                            </label>
                            <input className="w-full bg-white/5 border border-white/[0.06] rounded-xl px-4 py-3 text-sm text-white focus:border-[#c5a059]/50 outline-none transition-all"
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea className="w-full bg-white/5 border border-white/[0.06] rounded-xl px-4 py-3 text-sm text-slate-300 focus:border-[#c5a059]/50 outline-none transition-all resize-none" rows={6}
                    ...
                    <div className="bg-white/[0.02] border border-white/[0.04] rounded-xl p-5">
                        ...
                        <div className="flex flex-col md:flex-row gap-5">
                            {editContent.about.image ? (
                                ...
                                <div className="w-full md:w-64 h-44 rounded-xl border-2 border-dashed border-white/10 hover:border-[#c5a059]/40 flex flex-col items-center justify-center gap-2 cursor-pointer transition-all group/drop shrink-0"
                                    onDragOver={e => { e.preventDefault(); e.currentTarget.classList.add('border-[#c5a059]', 'bg-[#c5a059]/5'); }}
                                    onDragLeave={e => { e.preventDefault(); e.currentTarget.classList.remove('border-[#c5a059]', 'bg-[#c5a059]/5'); }}
                                    onDrop={e => { e.preventDefault(); e.currentTarget.classList.remove('border-[#c5a059]', 'bg-[#c5a059]/5'); const f = e.dataTransfer.files?.[0]; if (f?.type.startsWith('image/')) { const r = new FileReader(); r.onload = ev => setEditContent({ ...editContent, about: { ...editContent.about, image: ev.target?.result as string } }); r.readAsDataURL(f); } }}
                                    ...
                                    <i className="fa-solid fa-cloud-arrow-up text-xl text-slate-600 group-hover/drop:text-[#c5a059]"></i>
