...
export async function generateStaticParams() {
    ...
    return routing.locales.flatMap(locale =>
        content.regions
            ...
            .map(r => ({ locale, region: `${slugify(r.name)}-transfer` }))
    );
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const url = `${baseUrl}/${locale}/transfer/${region}`;
    ...
    routing.locales.forEach(l => { alternates[l] = `${baseUrl}/${l}/transfer/${region}`; });
...
export default async function TransferDestinationPage({ params }: Props) {
    ...
    const serviceSchema = {
        ...
        url: `${baseUrl}/${locale}/transfer/${region}`,
        provider: {
            '@type': 'LocalBusiness',
